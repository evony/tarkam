...
export async function GET() {
  ...
  for (const [, { player, clubs }] of playerClubMap) {
    if (clubs.length >= 2) {
      ...
      feedItems.push({
        ...
        subtitle: `${player.division === 'male' ? 'Divisi Male' : 'Divisi Female'}`,
        timestamp: player.updatedAt.toISOString(),
        division: player.division,
        accent: '#FF2D78',
      });
  ...
  for (const p of recentRegistrations) {
    ...
    feedItems.push({
      ...
      subtitle: `${p.division === 'male' ? 'Divisi Male' : 'Divisi Female'} • ${statusLabel}${p.city ? ' • ' + p.city : ''}`,
  ...
  for (const part of recentParticipations) {
    ...
    feedItems.push({
      ...
      subtitle: `Week ${t?.weekNumber || '?'} • ${t?.division === 'male' ? 'Male' : 'Female'} • ${statusLabel}`,
