...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: `Season "${season.name}" berhasil ditutup!${updateData.championPlayerId ? ' Champion otomatis ditentukan dari per-season points.' : updateData.championClubId ? ' Champion club otomatis ditentukan dari poin liga.' : ' Tidak ada champion yang ditentukan (belum ada point record).'}${skinAwardMessage} `,
      season: updated,
    });
