...
export function DreamSection({ maleData, femaleData, leagueData, nextSeason, completedSeason, cmsSections, cmsSettings, onEnterApp, openDonationModal, onVideoPlay }: DreamSectionProps) {
  ...
  return (<>
      ...
      <section id="dream" className="relative py-16 sm:py-28 px-4 overflow-hidden bg-deep">
        {/* Gold dot pattern */}
        <div className="absolute inset-0 opacity-[0.012]" style={{ backgroundImage: 'radial-gradient(circle, rgba(200,146,10,0.5) 1px, transparent 1px)', backgroundSize: '24px 24px' }} />
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at 50% 50%, rgba(200,146,10,0.07) 0%, transparent 50%), radial-gradient(ellipse at 20% 70%, rgba(46,159,255,0.03) 0%, transparent 40%), radial-gradient(ellipse at 80% 70%, rgba(255,45,120,0.03) 0%, transparent 40%)' }} />
        {/* Ambient orbs */}
        ...
        <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full border border-[#C8920A]/5" />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] h-[350px] rounded-full border border-[#C8920A]/8" />
        </div>
        ...
        >
          <div className="stagger-item-fast stagger-d0">
            <div className="flex items-center justify-center gap-3 mb-4">
              <div className="h-px w-12 sm:w-20 bg-gradient-to-r from-transparent to-[#C8920A]/50" />
              <div className="flex items-center gap-2 px-4 py-1.5 rounded-full border border-[#C8920A]/20 bg-[#C8920A]/5">
                <Trophy className="w-4 h-4 text-[#C8920A]" />
                <span className="text-[11px] font-bold text-[#C8920A] uppercase tracking-widest">Tarkam IDM</span>
              </div>
              <div className="h-px w-12 sm:w-20 bg-gradient-to-l from-transparent to-[#C8920A]/50" />
            </div>
          </div>
          ...
          {leagueData?.tarkamChampion && leagueData.tarkamChampion.seasonNumber !== 1 && (
            <div className="stagger-item-fast mt-6" style={{ animationDelay: '120ms' }}>
              <div className="inline-flex items-center gap-3 p-4 sm:p-5 rounded-2xl border border-[#C8920A]/20 bg-mid">
                <div className="relative">
                  <ClubLogoImage clubName={leagueData.tarkamChampion.name} dbLogo={leagueData.tarkamChampion.logo} alt={leagueData.tarkamChampion.name} width={40} height={40} className="w-10 h-10 rounded-2xl object-cover border border-[#C8920A]/30" />
                  <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-[#C8920A] flex items-center justify-center">
                    <Crown className="w-3 h-3 text-deep" />
                  </div>
                </div>
                <div className="text-left">
                  <p className="text-xs text-[#C8920A]/60 font-semibold uppercase tracking-wider">Season {leagueData.tarkamChampion.seasonNumber} Champion</p>
          ...
          <div className="stagger-item-fast mt-10 grid grid-cols-3 gap-3 sm:gap-4" style={{ animationDelay: '180ms' }}>
            {[
              { icon: Shield, value: `${clubsCompeting}`, label: 'Club Bertanding', accent: 'border-[#C8920A]/15' },
              { icon: Swords, value: `${matchesPlayed}`, label: 'Match Dimainkan', accent: 'border-[#C8920A]/10' },
              { icon: Users, value: `${totalParticipants}`, label: 'Peserta Total', accent: 'border-[#C8920A]/15' },
            ].map((s, i) => (
              <div key={s.label} className={`relative rounded-2xl bg-mid border ${s.accent} p-4 sm:p-5 transition-all duration-300 hover:bg-mid/80 hover:scale-[1.03] hover:shadow-[0_8px_32px_rgba(200,146,10,0.08)] group/stat`}>
                {/* Subtle radial glow behind icon */}
                <div className="absolute inset-0 rounded-2xl pointer-events-none opacity-0 group-hover/stat:opacity-100 transition-opacity duration-500" style={{ background: 'radial-gradient(ellipse at 50% 30%, rgba(200,146,10,0.05), transparent 70%)' }} />
                <s.icon className="w-4 h-4 text-[#C8920A] mx-auto mb-2 relative z-10 group-hover/stat:scale-110 transition-transform duration-300" />
          ...
          {countdown && cmsSettings?.countdown_label && (
            <div className="stagger-item-fast mt-8" style={{ animationDelay: '240ms' }}>
              <div className="rounded-2xl border border-[#C8920A]/20 bg-mid p-5 sm:p-6 text-center">
                <p className="text-xs text-[#C8920A]/70 font-bold uppercase tracking-widest mb-4">{cmsSettings.countdown_label}</p>
                  ...
                  {[
                    ...
                    <div key={unit.label} className="flex items-center gap-1.5 sm:gap-3">
                      <div className="flex flex-col items-center">
                        <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-lg sm:rounded-2xl bg-[#C8920A]/10 border border-[#C8920A]/20 flex items-center justify-center">
                          <span className="text-lg sm:text-2xl font-black text-[#C8920A] tabular-nums">{String(unit.value).padStart(2, '0')}</span>
                        </div>
                        <span className="text-[10px] text-muted-foreground/60 uppercase tracking-wider mt-1">{unit.label}</span>
                      </div>
                      {i < 3 && (
                        <span className="text-[#C8920A]/40 font-bold text-base sm:text-xl mb-4">:</span>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <div className="stagger-item-fast mt-8" style={{ animationDelay: '300ms' }}>
            <div className="rounded-2xl border border-[#C8920A]/15 bg-mid p-5 sm:p-6 text-center">
              <div className="flex items-center justify-center gap-2 mb-3">
                <Flame className="w-4 h-4 text-[#C8920A]" />
                <h4 className="text-sm font-bold text-[#C8920A]">Tarkam IDM Season {nextSeason}</h4>
          ...
          <div className="stagger-item-fast mt-8" style={{ animationDelay: '360ms' }}>
            <button onClick={() => openDonationModal('season')} className="hover-scale-md group/donate relative px-7 py-4 rounded-2xl bg-gradient-to-r from-[#C8920A] to-[#e8d5a3] text-deep font-black text-sm tracking-wider shadow-[0_0_30px_rgba(200,146,10,0.2)] hover:shadow-[0_0_60px_rgba(200,146,10,0.4)] transition-all cursor-pointer overflow-hidden">
      ...
      <section className="relative py-16 sm:py-24 px-4 overflow-hidden bg-deep">
        {/* Gold dot pattern */}
        <div className="absolute inset-0 opacity-[0.012]" style={{ backgroundImage: 'radial-gradient(circle, rgba(200,146,10,0.5) 1px, transparent 1px)', backgroundSize: '24px 24px' }} />
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at 50% 50%, rgba(200,146,10,0.06) 0%, transparent 50%)' }} />
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden" aria-hidden="true">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="absolute w-1 h-1 rounded-full bg-[#C8920A]/20"
              style={{
                left: `${15 + i * 15}%`,
                top: `${20 + (i % 3) * 25}%`,
        ...
        <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
          <div className="absolute top-8 left-8 w-16 h-16 border-l-2 border-t-2 border-[#C8920A]/10 rounded-tl-xl" />
          <div className="absolute top-8 right-8 w-16 h-16 border-r-2 border-t-2 border-[#C8920A]/10 rounded-tr-xl" />
          <div className="absolute bottom-8 left-8 w-16 h-16 border-l-2 border-b-2 border-[#C8920A]/10 rounded-bl-xl" />
          <div className="absolute bottom-8 right-8 w-16 h-16 border-r-2 border-b-2 border-[#C8920A]/10 rounded-br-xl" />
        </div>
        ...
        >
          <div className="animate-fade-enter">
            ...
            >
              <Sparkles className="w-10 h-10 text-[#C8920A]" />
            </div>
          </div>
