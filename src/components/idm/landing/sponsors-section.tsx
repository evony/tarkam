...
export function SponsorsSection() {
  ...
  return (
    ...
    >
      ...
      {/* ── Background ── */}
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at 50% 50%, rgba(200,146,10,0.05) 0%, transparent 60%)' }} />
          ...
          {sponsors.map((_, i) => (
            <button
              ...
              className={`compact-dot transition-all duration-300 rounded-full cursor-pointer ${
                i === activeIndex
                  ? 'bg-idm-gold-warm shadow-[0_0_4px_rgba(200,146,10,0.4)]'
...
function SponsorCardDesktop({ sponsor }: { sponsor: Sponsor }) {
  const logo = sponsor.logo;
  const inner = (
    <div className="group relative rounded-xl border border-idm-gold-warm/15 bg-idm-gold-warm/[0.04] h-28 overflow-hidden transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_0_24px_rgba(200,146,10,0.12)]">
      {logo ? (
        <>
          <Image
            src={logo}
